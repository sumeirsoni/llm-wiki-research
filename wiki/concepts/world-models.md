...
## Explanation-driven world models

[[learning-to-theorize|Learning-to-Theorize]] adds an explanation objective to the predictive world-model view. Its NEO model infers a latent executable program from an observation pair, then tests whether that program transfers to a new input generated by the same hidden mechanism. This makes program reuse an explicit evaluation target rather than treating target reconstruction as sufficient evidence of a useful world model.

The result complements the predictive and planning branches on this page. NEO uses controlled GridWorld, arithmetic, and image-editing tasks rather than real trajectories, and its primitive vocabulary is small and discrete. The paper therefore supports a research direction for explanatory world models, not a replacement for the long-horizon control results below.
